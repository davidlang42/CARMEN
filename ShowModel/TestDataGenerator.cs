...
namespace Carmen.ShowModel
{
    ...
    public class TestDataGenerator : IDisposable
    {
        ...
        public TestDataGenerator(ShowContext context, Random? random = null)
        {
            ...
            if (context.ShowRoot.ShowDate is DateTime show_date)
            {
                minimumDob = show_date.AddYears(-20);
                maximumDob = show_date.AddYears(-10);
            }
        }
        ...
        public void AddApplicants(uint count, Gender gender, bool include_criteria_abilities = true, bool include_cast_groups = true, bool include_tags = true)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                var applicant = new Applicant
                {
                    ...
                    Gender = gender,
                    DateOfBirth = RandomDate(random, minimumDob, maximumDob),
        ...
        private static uint TotalSections(uint sections_per_section, uint section_depth)
            => Convert.ToUInt32(Enumerable.Range(1, (int)section_depth).Select(d => Math.Pow(sections_per_section, d)).Sum());

        private static DateTime RandomDate(Random r, DateTime minimum, DateTime maximum)
        {
