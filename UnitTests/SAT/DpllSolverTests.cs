...
namespace UnitTests.SAT
{
    public class DpllSolverTests : SolverTests
    {
        ...
        [TestCase(100, 25, 110, 3, TestName = "All_25_Vars_VeryHard")] // 400ms
        //[TestCase(100, 50, 210, 3, TestName = "All_50_Vars_Extreme")] // 8.5s
        ...
        [Test]
        [TestCase(100, 10, 50, 3, TestName = "Count_10_Vars_Easy")] // 40ms
        [TestCase(100, 15, 70, 3, TestName = "Count_15_Vars_Medium")] // 100ms
        [TestCase(100, 20, 90, 3, TestName = "Count_20_Vars_Hard")] // 200ms
        public void CompareCounts(int test_cases, int n_variables, int j_clauses, int k_literals)
        {
            var vars = Enumerable.Range(1, n_variables).ToArray();
            var dpll = new DpllSolver<int>(vars);
            var brute = new BruteForceSolver<int>(vars);
            var maximum_solutions = 0;
            for (var seed = 0; seed < test_cases; seed++)
            {
                var expression = GenerateExpression(seed, vars, j_clauses, k_literals);
                var dpll_count = dpll.Solve(expression).SelectMany(s => s.Enumerate()).Count();
                var brute_count = brute.Solve(expression).SelectMany(s => s.Enumerate()).Count();
                var dpll_all = dpll.Solve(expression).SelectMany(s => s.Enumerate()).ToArray();
                var brute_all = brute.Solve(expression).SelectMany(s => s.Enumerate()).ToArray();
